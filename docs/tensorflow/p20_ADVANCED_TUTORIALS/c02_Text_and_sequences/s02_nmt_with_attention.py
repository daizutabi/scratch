...
# ## Download and prepare the datasetetup
# !Download the file
...
_ = train_test_split(input_tensor, target_tensor, test_size=0.2)
input_tensor_train, input_tensor_val, target_tensor_train, target_tensor_val = _
...
checkpoint_dir = cache_dir("tensorflow/nmn_with_attention/training_checkpoints")
...
result, sentence, attention = evaluate(u"hace mucho frio aqui.")
print(sentence)
print(result)
xt = sentence.split(" ")
yt = result.split(" ")
attention = attention[: len(yt), : len(xt)]
x, y = np.meshgrid(xt, yt)
df = pd.DataFrame({"x": x.ravel(), "y": y.ravel(), "z": attention.ravel()})
alt.Chart(df).mark_rect().encode(
    x=alt.X("x:O", sort=None), y=alt.Y("y:O", sort=None), color="z:Q"
)
